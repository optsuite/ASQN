...
activeMask = true(blockSize,1); Numcount = 1;
...
for iterationNumber=1:maxIterations
    ...
    residualNorms=full(sqrt(sum(conj(blockVectorR).*blockVectorR)'))./max(1, abs(lambda));
    ...
    Numcount = Numcount + currentBlockSize/blockSize;
...
varargout(3)={residualNormsHistory(1:blockSize,1:iterationNumber)};
varargout(4)={Numcount};
end